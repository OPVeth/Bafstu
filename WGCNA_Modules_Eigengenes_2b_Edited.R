...
createTopologyThreshold <- function(sft, tissue){
    ...
    par(mfrow = c(1,2))
    cex1 = 0.9
    pdf(paste("Topology_Connectivity_",tissue,"WT.pdf", sep=""))
...
doClustering <- function(dissTOM, tissue){
    ...
    sizeGrWindow(12,9)
    pdf(paste("Dissimilarity_Matrix_",tissue,"WT.pdf", sep=""))
...
createDendogram <- function(dissTOM, geneTree, deepSplit, cutHeight, minModuleSize, tissue){
    ...
    sizeGrWindow(8,6)
    pdf(paste("Gene_Dendogram_Module_Colors_",tissue,"_", minModuleSize,"WT.pdf", sep=""))
...
doClusteringMEs <- function(MEs, minModuleSize, tissue){
    ...
    sizeGrWindow(7, 6)
    pdf(paste("Clustering_Module_Eigengenes_",tissue,"_",minModuleSize,"WT.pdf", sep=""))
...
saveResults <- function(MEs, moduleLabels, moduleColors, geneTree, tissue){
    ...
    save(MEs, moduleLabels, moduleColors, geneTree, 
         file = paste("Mice-02-networkConstruction_",tissue,"WT.RData", sep=""))
}
...
main <- function(){
    ...
    for (x in 1:length(tissues)){
      load(file = paste("Mice-01-dataInput-",tissues[x],"WT.RData", sep=""))
