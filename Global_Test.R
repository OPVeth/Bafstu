...
library(tidyr)
library(globaltest)
setwd("~/Downloads/Test_Scripts")#Directory of interest
...
getPathways <- function(dataTissue, dataPathwayGenes){
  ...
  #   Thereafter, a for-loop checks if any of the rows in a column contains NA
  #   values. If that's not the case, the genes in dataPathwayGenes
...
performGTPathway <- function(pathwaysTissue, dataTissue, tissue){
  ...
  #for (x in length(pathwaysTissue)){
    ...
    doGTPathway("WT", "mdx+-", pathwaysTissue, dataTissue, tissue)
    doGTPathway("WT", "mdx++", pathwaysTissue, dataTissue, tissue)
    doGTPathway("mdx", "mdx+-", pathwaysTissue, dataTissue, tissue)
    doGTPathway("mdx", "mdx++", pathwaysTissue, dataTissue, tissue)
    doGTPathway("mdx+-", "mdx++", pathwaysTissue, dataTissue, tissue)
   # }
}
...
doGTPathway <- function(genotype1, genotype2, pathwaysTissue, dataTissue, tissue){
  ...
  # pathwaysTissue, the position and dataTissue is given to saveResultsGT().
  # Thereafter a regular GT() is performed on every pathway. A Holm correction is
  # performed on the global test data and saved 
  ...
  for (x in 1:length(pathwaysTissue)){
    gtResults <- gt(as.factor(dataTissue$Group), dataTissue[,1:ncol(dataTissue)-1], permutations=10000, subsets = pathwaysTissue[[x]], model = "multinomial")
  ...
  gtResults <- gt(as.factor(exactRows), dataTissue[positions, 1:ncol(dataTissue)-1], permutations=10000, subsets = pathwaysTissue)
...
saveResultsGT <- function(genotype1, genotype2, pathway, pos, dataTissue){
  ...
  # Thereafter gt() is performed with exactRowsSecond, the right rows and every
  # column except the last of dataTissue and
...
main <- function(){
  ...
  for(x in 1:length(tissues)){
    if (tissues[x] == "Blood"){
      traitData <- read.csv2("Spitali_Neuromics.csv")[1:96,]
    } else{
      traitData <- read.csv2("Spitali_Neuromics.csv")[97:147,]
    }
    dataTissue <- data.frame(read.csv2(paste("Count_",tissues[x], "_Normalized_Log_Transformed.csv", sep="")))
