...
createHeatmap <- function(moduleTraitCor, nSamples, tissue, MEs, datTraits){
    ...
    par(mar = c(6, 8.5, 3, 3)) 
    pdf(file = paste("Module_Trait_Relationship_",tissue,"_.pdf"), wi = 9, he = 6)
...
createHeatmapGenotype <- function(datTraits, tissue, MEs, datExpr){
  ...
  textMatrix <- rbind(sigMat)
  textMatrix <- rbind(sigMat, textMatrix)
  mat <- rbind((sigMat < 0.05) * 1, mat)
  print(colnames(MEs))
  cols <- c(apply(colnames(MEs), 2, function(c) paste(c[1], 'vs', c[2])))
  par(mar = c(10, 10.5, 6, 6))
  pdf(file = paste("Module_Trait_Relationship_",tissue,".pdf", sep=""), wi = 9, he = 6)
...
createHeatmapMEs <- function(datTraits, tissue, MEs, datExpr){
  ...
  traitData <- read.csv2("~/Dropbox/Bafstu/Spitali_Neuromics.csv")
  ...
  par(mar = c(6, 8.5, 3, 3)) 
  pdf(file = "Module_Trait_Relationship_Blood_MinMod30_MEDiss_015_MDX_Muscle_ME.pdf", wi = 9, he = 6)
...
saveGeneList <- function(datExpr, moduleColors, module, tissue, genes){
    ...
    for (x in 1:length(genes)){
      ...
      if (length(location) > 1){
        genes[x] <- as.character(mapData$ENTREZ[location[1]])
      } else if (length(location) == 1) {
        genes[x] <- as.character(mapData$ENTREZ[location]) 
      }
    }
    genes <- genes[!is.na(genes)]
    write(genes, file=paste("Module_",module,"_",tissue,"_", hub, "Genelist.txt", sep=""))
}
...
main <- function(){
    # Two vectors are created, containing two string objects. 
    #   For each tissue, its RData file of the first and second script
    #   is loaded in.
    #   Afterwards, the amount of rows in datExpr is saved within a variable,
    #   'nSamples'. The function 'orderModuleEigengenes' gets called with 
    #   'datExpr' and 'moduleColors'. 
    #   The correlation of The resulting list of modules with datExpr gets
    #   calculated. 
    #   Thereafter, the 'mdx' column of traitData is selected and turned into
    #   a data frame. The column names are changed into 'weight' and the module
    #   names itself are saved with a variable. 
    #   Thereafter, a heatmap is created of the correlation matrix, nSamples 
    #   and the tissue. 
    #   Another function gets called, 'getCorTrait', with datTraits, MEs, datExpr,
    #   nSamples and weight which results in a correlation matrix of the gene
    #   expression data and traits. 
    #   The function 'getCorME' is called with  'datExpr', 'MEs', 'nSamples' and
    #   'modNames', resulting in a correlation matrix between the expression data
    #   and module eigen genes.
    #   The module which name matched with a index within the moduleNames gets
    #   retrieved and also the genes of the module itself. 
    #   These variables are given to the makeScatterPlot function with the tissue 
    #   as a String object, geneModuleMembership, geneTraitsSignificance and the module name.
    #   The function 'calculateMean()' is called with 'moduleColors' and 'datExpr'.
    #   Afterwards, 'saveGeneList' is called with 'datExpr', 'moduleColors','module' and tissue as a String object. 
    ...
    for(x in 1:length(tissues)){
      ...
      #createHeatmapGenotype(datTraits, tissues[x], MEs, datExpr)
      createHeatmapMEs(datTraits, tissues[x], MEs, datExpr)
      weight <- as.data.frame(datTraits$mdx) 
      names(weight) = "weight"
      modNames <- substring(names(MEs), 3)
      print("Creating heatmap...")
      #createHeatmap(moduleTraitCor, nSamples, tissues[x], MEs, datTraits)
      #print("Calculating correlation between expression data and trait...")
      geneTraitsSignificance <- getCorTrait(datTraits, MEs, datExpr, nSamples, weight)
      print("Calculating correlation between expression data and module eigengenes..")
      column <- match(modules, modNames)
      moduleGenes <- moduleColors==modules
      geneModuleMembership <- getCorME(datExpr, MEs, nSamples, modNames)
      geneModuleMembership <- data.matrix(abs(geneModuleMembership))
      geneSpecModuleMembership <- geneModuleMembership[order(geneModuleMembership[, column]), column][1:5]
      hubGenes <- names(geneSpecModuleMembership)
      column <- match(modules[x], modNames)
      moduleGenes <- moduleColors==modules[x]
      print(moduleGenes)
      #print("Creating scatterplot of module...")
      #makeScatterPlot(tissues[x], geneSpecModuleMembership, moduleGenes, column, 
      #                geneTraitsSignificance, modules[x])
      #print("Calculating mean of module sizes...")
      #calculateMean(moduleColors, datExpr)
      #print("Saving resulting genes of module...")
      saveGeneList(datExpr, moduleColors, modules[x], tissues[x], "")
      saveGeneList(datExpr, moduleColors, modules[x], tissues[x], hubGenes)
      writeCountData(datExpr, hubGenes, modules[x], tissues[x], datTraits)
    }
}
main()
