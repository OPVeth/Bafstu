...
doClustering <- function(dataTissue, tissue){
    ...
    load(paste("genotypes_",tissue,".RData",
               sep=""))
...
changeClusters <- function(dataTissue, treeTissue, cutHeight){
    ...
    table(clust)
    keepSamples <- (clust==1) #Depended on the cut
...
main <- function(){
    ...
    for (x in 1:length(tissues)){
      ...
      miceData  <- read.csv2(paste("Count_",tissues[x],"_Normalized.csv", sep=""))
      print("Preprocessing...")
