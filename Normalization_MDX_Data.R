...
getTrait <- function(traitDataTissue, tissue, miceData){
    ...
    save(genotypes, file=paste("genotypes_", tissue, ".RData", sep=""))
    return(traitData)
}
...
getNormalizedData <- function(dataTissue, tissue){
    ...
    save(tmm, file=paste("Gene_names_", tissue, ".RData", sep=""))
...
saveNormalData <- function(tissueDataNormal, tissue){
  ...
  write.csv2(tissueDataNormal, paste("Count_",tissue,"_Normalized.csv", sep=""))
...
main <- function(){
    ...
    miceData <- preProcessingData(read.csv2("batch1_batch2_all.counts.tsv", sep=" "))
    ...
    for (x in 1:length(tissues)){
      ...
      saveNormalData(normalizedData, tissues[x])
    }
}
main()
# Session Information
